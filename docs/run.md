...
AEM **page-shell** components (the components used as `sling:resourceType` on `jcr:content` — `/apps/<site>/components/structure/page` and friends) are declared in `aem-page-components.json` paired with the `cq:template` paths each is authored under. Each pair becomes one Sanity document type whose `pageProperties` field carries the page-shell dialog's authored values (`pwaOrientation`, `disableCache`, …) lifted from `jcr:content`. `cq:featuredimage` becomes a `featuredImage` Sanity image; bookkeeping (replication agents, versioning) is dropped. Pages with declared resource types but undeclared templates fall back to the generic `page` doc and surface as `unknownPageTemplates` findings. Empty file → no per-template docs (today's behavior). See running-the-migration § 1c-septies for the config shape.
